...
public class Game {
	...
	public Game(TimeBoard timeBoard, OpponentHandler playerHandler, PieceHandler pieceHandler, CardHandler cardHandler,
		...
		Objects.requireNonNull(cardHandler);
	...
	/**
	 * The function that launches the correct version of the game
	 */
	public void launch() {
			...
			if (action == Action.KEY_PRESSED) {
				if (event.getKey().equals(KeyboardKey.LEFT) && y > 0) {
					y--;
						...
						&& (y + piece.getXSize()) < Constants.GRID_SIZE.getValue()) {
					y++;
				}
	...
	private void automaTurn() {
		...
		piece = automa.buyingPhase(listBuyablePieces, _opponentHandler.getHumanPlayer().getPosition(), automa.getPosition(),
				card, _timeBoard);
		if (piece != null) {
			_pieceHandler.moveNeutralPawn(_pieceHandler.getSelectablePieces().indexOf(piece));
			_pieceHandler.remove(piece);
		}
		// test(card, listBuyablePieces, piece, automa.getPosition(),
		// _opponentHandler.getHumanPlayer().getPosition());
	...
	/**
	 * The function calls the function to add the piece to the grid and to do the
	 * necessary changes after a piece has been purchased return a boolean that
	 * indicates if the piece was bought
	 * 
	 * @param context
	 * @param pieceNumber
	 * @return boolean
	...
	/**
	 * Handles a turn of a player.
	 * @param context : Object used to exit the window.
	 * @param event : used to get the inputs of the player.
	 * @param played : used to know if we have to update the special tile and position of the player.
	 * @param pieceNumber : number of the selected piece
	 * @return int to update the pieceNumber
	 */
	...
	/**
	 * The function draw the different options available if the player is in the
	 * special box DRAW
	 * 
	 * @param graphics
	 * @param height
	...
	private void drawOptions(Graphics2D graphics, float height, float width, Piece pieces[]) {
		...
		graphics.draw(rec);
		for (var i = 0; i < 3; i++) {
			graphics.draw(new Line2D.Double(width / 2 - width / 10, i * size + height / 2 - width / 5,
					width / 2 - width / 10 + size * 3, i * size + height / 2 - width / 5));
			graphics.draw(new Line2D.Double(width / 2 - width / 10 + i * size, height / 2 - width / 5,
					width / 2 - width / 10 + i * size, size * 3 + height / 2 - width / 5));
		}
	...
	/**
	 * The function allows the player to choose the piece he wants to add to his
	 * grid (4blocs max)
	 * 
	...
	public Piece chooseOptionalPiece(ApplicationContext context, IOpponent player) {
		...
		while (true) {
			...
			context.renderFrame(graphics -> {
				drawOptions(graphics, height, width, pieces);
				graphics.setColor(Color.MAGENTA);
				var rectangle = new Rectangle2D.Float(width / 2 - width / 10 + toto * size,
						height / 2 - width / 5 + tutu * size, size, size);
				graphics.draw(rectangle);
			});
		}
	}
	...
	private void draw(Graphics2D graphics, float height, float width, int pieceNumber, float pieceHandlerPos,
		...
		_opponentHandler.drawAutomaStats(graphics);
		_cardHandler.draw(graphics);
		_menu.draw(graphics);
	...
	/**
	 * The function draws the current state of the player's timeBoard with the menu
